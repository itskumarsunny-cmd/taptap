...
// ── EDIT PROFILE (username / gender / country) ────────────────
app.patch('/api/profile', authMiddleware, async (req, res) => {
  try {
    const { username, gender, countryCode } = req.body;
    const user = await User.findById(req.userId);
    if (!user) return res.status(404).json({ error: 'User not found' });

    if (username !== undefined && username !== user.username) {
      if (username.length < 3) return res.status(400).json({ error: 'Username must be at least 3 characters' });
      if (!/^[a-zA-Z0-9_]+$/.test(username)) return res.status(400).json({ error: 'Username: only letters, numbers, underscores' });
    ...
    if (gender !== undefined) {
      if (!['male', 'female', 'other'].includes(gender)) return res.status(400).json({ error: 'Please select a valid gender' });
      user.gender = gender;
    }

    if (countryCode !== undefined) {
      const country = COUNTRY_MAP.get(countryCode);
      if (!country) return res.status(400).json({ error: 'Please select a valid country' });
...
// ── PROFILE PICTURE ──────────────────────────────────────────
// Stored as a compressed base64 data URI — the frontend resizes/compresses
// client-side before sending, so this is just a safety cap (~700KB raw).
const MAX_AVATAR_LEN = 950000;
...
app.post('/api/profile/avatar', authMiddleware, async (req, res) => {
  try {
    ...
    if (image.length > MAX_AVATAR_LEN) {
      return res.status(400).json({ error: 'Image is too large. Please choose a smaller photo.' });
    }

    const user = await User.findByIdAndUpdate(req.userId, { avatarUrl: image }, { new: true });
    if (!user) return res.status(404).json({ error: 'User not found' });
...
app.delete('/api/profile/avatar', authMiddleware, async (req, res) => {
  try {
    const user = await User.findByIdAndUpdate(req.userId, { avatarUrl: null }, { new: true });
    if (!user) return res.status(404).json({ error: 'User not found' });
    res.json({ success: true, user: toPublicUser(user) });
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
...
app.get('/api/history', authMiddleware, async (req, res) => {
  try {
    const raw = await MatchHistory.find({ userId: req.userId }).sort({ matchedAt: -1 }).limit(200);

    // Keep only the most recent entry per partner (raw is already sorted newest-first)
    const seen = new Set();
    const deduped = [];
    for (const h of raw) {
      const key = h.partnerId ? String(h.partnerId) : `unknown-${h._id}`;
    ...
    const partnerIds = [...new Set(deduped.map(h => h.partnerId).filter(Boolean).map(String))];
    const partners = await User.find({ _id: { $in: partnerIds } }).select('avatarUrl');
    const avatarById = new Map(partners.map(p => [String(p._id), p.avatarUrl || null]));

    res.json({
      history: deduped.map(h => ({
        partnerId: h.partnerId, partnerName: h.partnerName,
        partnerAvatar: h.partnerId ? (avatarById.get(String(h.partnerId)) || null) : null,
  ...
  try {
    ...
    const list = convos.map(c => {
      ...
      return {
        ...
        partnerAvatar: partner ? (partner.avatarUrl || null) : null,
...
io.on('connection', (socket) => {
  ...
  socket.on('find-partner', async ({ mode, genderFilter, countryFilter }) => {
    ...
    if (idx !== -1) {
      ...
      socket.emit('partner-found', { partnerName: partner.username, partnerId: partner.userId, partnerAvatar: partner.avatarUrl || null, isInitiator: true, mode: me.mode });
      io.to(partner.socketId).emit('partner-found', { partnerName: me.username, partnerId: me.userId, partnerAvatar: me.avatarUrl || null, isInitiator: false, mode: me.mode });
      ...
      waitingQueue.push({
        ...
        gender: me.gender, countryCode: me.countryCode, avatarUrl: me.avatarUrl,
  ...
  socket.on('private-call', async ({ targetUserId, callType }) => {
    ...
    io.to(targetSocketId).emit('incoming-call', {
      ...
      callerName: me.username, callerUserId: me.userId, callerAvatar: me.avatarUrl || null
    });
  });
